...
export default function FloatingWorkoutControls() {
  ...
  useEffect(() => {
    if (activeSession) timerRef.current = setInterval(() => { if (!paused) setTimer((t) => t + 1) }, 1000)
    return () => { clearInterval(timerRef.current) }
  }, [activeSession, paused])

  useEffect(() => { setTimer(0); setPaused(false); setRestTimer(null); clearInterval(restRef.current) }, [activeSession?.exerciseId])
  ...
  const startRest = (d) => {
    ...
    setRestTimer(d)
    restRef.current = setInterval(() => setRestTimer((t) => (t <= 1 ? null : t - 1)), 1000)
  }

  if (!activeSession) return null

  const f = (s) => { if (s === null || s === undefined) return '0:00'; const m = Math.floor(s / 60); const sec = s % 60; return `${m}:${sec.toString().padStart(2, '0')}` }

  return (
    <div className="fixed bottom-24 left-3 right-3 z-50 max-w-lg mx-auto">
      {restTimer !== null && (
        <div className="mb-2 flex justify-center">
          <div className="flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-dark-card/95 backdrop-blur-xl border border-lime/30 shadow-2xl">
      ...
      {collapsed ? (
        <button onClick={() => setCollapsed(false)} className="ml-auto flex items-center gap-2 px-4 py-2 rounded-full bg-dark-card border border-lime/30 shadow-xl">
          ...
          <span className="text-white text-sm font-medium">{f(timer)}</span>
        </button>
      ) : (
        <div className="bg-dark-card/95 backdrop-blur-xl rounded-2xl p-4 border border-lime/30 shadow-2xl shadow-black/30">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${paused ? 'bg-yellow-400' : 'bg-lime animate-pulse'}`} />
          ...
          <div className="flex gap-1.5 mb-2">
            {[30, 60, 90, 120].map((d) => (
              <button key={d} onClick={() => startRest(d)} className="flex-1 py-1.5 rounded-lg text-[10px] font-medium bg-white/5 text-white/50 hover:bg-white/10 border border-transparent">{d}s</button>
            ))}
          </div>
