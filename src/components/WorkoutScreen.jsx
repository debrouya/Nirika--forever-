...
export default function WorkoutScreen({exercise,onComplete}){
  ...
  const end=()=>{console.log('[WorkoutScreen] end() -> onComplete, currentView=', useStore.getState().currentView);setConfirmQuit(false);clearInterval(iv.current);if(wl.current)wl.current.release().catch(()=>{});onComplete()}
  ...
  return(
    <div className="fixed inset-0 z-40 flex flex-col" style={{backgroundColor:bg}}>
      ...
      <div className="px-4 flex gap-3" style={{marginBottom:'calc(env(safe-area-inset-bottom, 20px) + 80px)'}}>
